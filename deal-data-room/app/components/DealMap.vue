...
function addMarkers() {
  ...
  const validDeals = props.deals.filter(d => d.lat && d.lng)
  if (!validDeals.length) return
  ...
  validDeals.forEach((deal) => {
    ...
    L.marker([deal.lat, deal.lng], { icon: svgIcon })
      .addTo(markersLayer)
      .bindPopup(popupHtml, { maxWidth: 280, className: 'deal-popup' })

    bounds.push([deal.lat, deal.lng])
  })
...
onMounted(async () => {
  ...
  // Add markers for any deals already loaded
  addMarkers()
})

// Re-add markers whenever deals data arrives/changes (Supabase loads async)
watch(() => props.deals, () => addMarkers(), { deep: true })
